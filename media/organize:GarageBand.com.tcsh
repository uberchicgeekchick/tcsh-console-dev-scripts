...
foreach title ( "`/usr/bin/find Genres -iregex '.*, released on.*\.\(mp.\|ogg\|flac\|m4a\)'`" )
	set pubdate="`printf "\""${title}"\"" | sed 's/Genres\/\([^\/]\+\)\/\(.*\)\ by\ \(.*\)\(, released on[^\.]*\)\.\(mp.\|ogg\|flac\|m4a\)/\4/g'`";
	if( "${pubdate}" == "" || "${pubdate}" == "${title}" )	\
		continue;
	set genre = "`printf "\""${title}"\"" | sed 's/Genres\/\([^\/]\+\)\/\(.*\)\ by\ \(.*\)\(, released on[^\.]*\)\.\(mp.\|ogg\|flac\|m4a\)/\1/g'`";
	set song = "`printf "\""${title}"\"" | sed 's/Genres\/\([^\/]\+\)\/\(.*\)\ by\ \(.*\)\(, released on[^\.]*\)\.\(mp.\|ogg\|flac\|m4a\)/\2/g'`";
	set artist = "`printf "\""${title}"\"" | sed 's/Genres\/\([^\/]\+\)\/\(.*\)\ by\ \(.*\)\(, released on[^\.]*\)\.\(mp.\|ogg\|flac\|m4a\)/\3/g'`";
...
foreach title ( "`/usr/bin/find Genres -iregex '.*\.\(mp.\|ogg\|flac\|m4a\)'`" )
	set song = "`printf "\""${title}"\"" | sed 's/.*\/\(.*\)\ by\ \(.*\)\.\(mp.\|ogg\|flac\|m4a\)/\1/g'`";
	set artist = "`printf "\""${title}"\"" | sed 's/.*\/\(.*\)\ by\ \(.*\)\.\(mp.\|ogg\|flac\|m4a\)/\2/g'`";
	set extension = "`printf "\""${title}"\"" | sed 's/.*\.\(mp.\|ogg\|flac\|m4a\)/\1/g'`";
