...
foreach episode("`/usr/bin/find -L "\""${target_directory}"\"" -regextype posix-extended -iregex '.*\.([^\.]+)"\$"' -type f | sort | sed -r 's/^\ //' | sed -r 's/(["\""])/"\""\\"\"""\""/g' | sed -r 's/["\$"]/"\""\\"\$""\""/g' | sed -r 's/(['\!'])/\\\1/g'`")
	...
	set title="`printf "\""${episode}"\"" | sed -r 's/^(.*)\/(.*)(,\ released\ on\:\ [^\.]*)\.([^\.]+)"\$"/\1\/\2\.\4/g'`";
	#echo $title;
	
	set books_chapter_or_episode_string="`printf "\""${title}"\"" | sed -r 's/(.*)\/(.*)([CE])?(hapter|pisode)?[^\.]*[^\.]+\.([^\.]+)/\u\3\L\4/ig' | sed -r 's/^0//'`";
	if( "${books_chapter_or_episode_string}" == "" || "${books_chapter_or_episode_string}" == "${title}" ) then
		set books_chapter_or_episode_string="Episode";
		#printf "Cannot rename <${episode}> it does not appear to be a numbered episode or chapter release.\n" > /dev/stderr;
