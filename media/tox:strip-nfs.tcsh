...
if( ! ( ${?1} && "${1}" != "" && "`printf "\""${1}"\"" | sed 's/.*\(\.tox\)${eol}/\1/'`" == ".tox" && -e "${1}" ) ) then
	printf "Usage: %s playlist.tox [toxine/playlist/filename.tox]" "`basename '${0}'`";
...
if( ! ( ${?2} && "${2}" != "" && "`printf "\""${2}"\"" | sed 's/.*\(\.tox\)${eol}/\1/g'`" == ".tox" ) ) then
	set playlist="`printf "\""${1}"\"" | sed 's/\(.*\)\([^\/]\+\)\(\.tox\)${eol}/\1\2\.local\.tox/'`";
else
	set playlist="${2}";
endif
if( "${1}" == "${playlist}" ) then
...
ex -E -n -s -X '+3,$s/^\(\tmrl\ =\ \/media\/podcasts\)\/nfs\/\(.*\)\/\([^\/]\+\)\.\([^\.]\+\);$/\1\/\2\/\3\.\4;/' '+wq!' "${playlist}";
