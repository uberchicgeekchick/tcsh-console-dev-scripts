...
<?php
	...
	function recurse_path( $pwd ) {
		if(!(is_dir($pwd)))
			return "";
		
		$path=":".$pwd;
		
		$dir_pointer=opendir($pwd);
		while( ($dir=readdir($dir_pointer)) )
			if( (!(preg_match("/^\./", $dir)))&&($dir=$pwd."/".$dir)&&(is_dir($dir)) )
				$path.=recurse_path($dir);
		closedir($dir_pointer);
		return $path;
	}
	
	if(!( (isset($_SERVER['argv'][1]))&&(is_dir($_SERVER['argv'][1])) )) {
		fprintf(STDERR, (utf8_encode( (sprintf("Usage: %s [Directories]", (basename($_SERVER['argv'][0])) )) )) );
		return getenv("PATH");
	}
	
	printf( (utf8_encode( (getenv('PATH')).(recurse_path( (preg_replace( "/\/$/", "", $_SERVER['argv'][1] )) )) )) );
?>
