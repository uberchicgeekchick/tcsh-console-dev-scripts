...
parse_argv:
	...
	while( $arg < $argc )
		...
		set commaand_to_parse="$argv[$arg]";
		set callback="parse_command";
		goto callback_handler;
	end
...
parse_command:
	set label_current="parse_command";
	if(! ${?label_previous} ) then
		goto callback_stack_update;
	else if("${label_current}" != "${label_previous}") then
		goto callback_stack_update;
	...
	printf "%s" "${commaand_to_parse}" >! "${argument_file}";
...
read_stdin_init:
	...
	set arg_handler="check_arg";
	set arg_parse_complete="read_stdin_quit";
...
read_stdin:
	...
	while( "${value}" != "" )
		...
		set commaand_to_parse="$<";
		#set commaand_to_parse=$<:q;
		set callback="parse_command";
		goto callback_handler;
	end
	set value="$<";
